...
def plot(df):
    ...
    df_pivot = df_filtered.pivot(index="geo", columns="lev_limit", values="car_rate_%")
    ...
    plt.xlabel("Cannot afford a personal car (%)")
    plt.title(
        "Disability Gap: Inability to afford a personal car (2021)\n(Some/Severe limitations vs No limitations, age 16+, total)"
    )
    plt.legend()
    plt.tight_layout()
